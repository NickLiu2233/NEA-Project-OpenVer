...
function applyBackendCompatPatch(bundlePath) {
  ...
  const cookiePatch = readFileSync(join(__dirname, "backend-cookie-eviction.patch"), "utf8");
  const finalOutput = applyUnifiedPatch(output, cookiePatch);
  assertHash(finalOutput, FINAL_SHA256, "backend cookie eviction patch output");
  writeFileSync(bundlePath, finalOutput);
}
